    ...
    <div className="page-view callup-page" style={{display: 'flex', flexDirection: 'column', height: '100%', overflow: 'hidden', background: 'var(--bg-1)'}}>
      ...
      <div className="callup-body" style={{padding: '30px', overflowY: 'auto', flex: 1}}>
        ...
        {loading ? <div className="callup-msg">Loading…</div> : (
          <div className="camp-card-grid" style={{display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(320px, 1fr))', gap: '20px'}}>
              ...
              editingId === camp.id ? (
                ...
                <div key={camp.id} className="camp-card" onClick={() => setActive(camp.id)} 
                  ...
                  {fmtDateRange(camp.camp_date, camp.camp_date_end) && (
                    <div style={{color: 'var(--fg-dim)', fontSize: 13, display: 'flex', alignItems: 'center', gap: 5}}>
                      📅 {fmtDateRange(camp.camp_date, camp.camp_date_end)}
                    </div>
                  )}

                  <div style={{marginTop: 20, paddingTop: 15, borderTop: '1px solid var(--line-soft)', display: 'flex', justifyContent: 'space-between', alignItems: 'center'}}>
                    <div style={{fontSize: 13, color: 'var(--fg-dim)'}}>
                      <strong style={{color: 'var(--fg)', fontSize: 16}}>{camp.playerIds?.length || 0}</strong> Players Called
                    </div>
